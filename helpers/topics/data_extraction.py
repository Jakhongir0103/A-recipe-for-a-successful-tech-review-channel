...
class DataExtractor():
    def __init__(self, config, video_data) -> None:
        self.config = config
        self.video_data = video_data.copy()
        self.channel_data = pd.read_csv(config['channel_data_path'], sep='\t', compression='infer')
    ...
    def get_topic_info(self, data: pd.DataFrame, release: str, start: str, end: str, topic: list = None) -> dict:
        """
        ...
        if topic:
            df = df[
                (df['title'].str.contains('|'.join(topic), case=False)) & 
                (df['upload_date'] > datetime.datetime.strptime(start, '%Y-%m-%d')) & 
                (df['upload_date'] < datetime.datetime.strptime(release, '%Y-%m-%d'))
            ]
        ...
        channels_df = channels_df.groupby('datetime').sum().reset_index()
        channels_df.sort_values('datetime',ascending=True,inplace=True)
        ...
        channels_df['ratio_views'] = channels_df['ratio_views'].rolling(window=5).mean()
        channels_df['ratio_subs'] = channels_df['ratio_subs'].rolling(window=5).mean()

        channels_df['datetime'] = pd.to_datetime(channels_df['datetime']).dt.date
