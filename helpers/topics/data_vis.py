...
def plot_product_hist(x, y, release_date, title, ax):
    ...
    ax.axvline(dt.datetime.strptime(release_date, '%d-%m-%Y'), color='red', linewidth=1, linestyle='--', label=f'Release date: {release_date}')
    ...
    legend_elements = [ax.bar(x=[0], height=0, width=0, color=color_palette[col], alpha=0.7, label=f'Color {col}')[0] for col in np.unique(colors)]
    legend_elements.append(ax.axhline(0, color='black', linewidth=0.5))
    ax.legend(legend_elements, ['Before release', 'Release', 'After release', 'Release date'])
...
def plot_line(data_dict: dict, release: str, title: str):
    ...
    # Convert release_date to timestamp format
